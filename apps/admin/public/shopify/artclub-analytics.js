(function () {
  ...
  function sendGaEvent(payload) {
    if (typeof window.gtag !== "function" || !payload || !payload.eventType) return;
  ...
  function observeImpressionTargets(root) {
    ...
    if (scope.nodeType === 1 && scope.hasAttribute && scope.hasAttribute("data-artclub-track-impression")) {
      if (!sentImpressions.has(scope)) observer.observe(scope);
    }
  ...
  function setupDynamicObservation() {
  ...
  function findArtistViewTarget() {
    return document.querySelector("[data-artclub-track-view='artist']") || document.querySelector("[data-artclub-track-impression='artist']");
  }

  function trackInitialViews() {
    var context = pageContext();
    var hasEmbedContainers = document.querySelector("[data-artclub-artist-embed]") !== null;
    var artworkTarget = document.querySelector("[data-artclub-track-impression='artwork']");

    if (!hasEmbedContainers) {
      var artistTarget = findArtistViewTarget();
      if (artistTarget) {
  ...
  function setupArtistEmbedTracking() {
    document.addEventListener("artclub:artist-embed-rendered", function (event) {
      ...
      var artist = detail.payload || {};
      if (!container) return;

      observeImpressionTargets(container);
      sendPayload(
        payloadForEvent("artist_profile_view", container, {
