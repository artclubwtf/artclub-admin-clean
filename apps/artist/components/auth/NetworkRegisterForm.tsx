...
export function NetworkRegisterForm() {
  ...
  return <form onSubmit={submit} className="w-full max-w-md space-y-5"><div><p className="eyebrow">ARTCLUB – The network of art</p><h1 className="mt-2 text-4xl font-semibold tracking-tight">Create account</h1></div>{[["name","Name","text"],["email","Email","email"],["password","Password (10+ characters)","password"]].map(([key,label,type])=><label key={key} className="block"><span className="mb-1 block text-sm">{label}</span><input required disabled={isSubmitting} type={type} value={(value as any)[key]} onChange={event=>setValue({...value,[key]:event.target.value})} className="control w-full disabled:opacity-60"/></label>)}{error&&<p role="alert" className="text-sm text-[var(--danger)]">{error}</p>}<button type="submit" disabled={isSubmitting} className="primary-action w-full disabled:opacity-50">{isSubmitting?"Creating account…":"Create account"}</button></form>;
}
