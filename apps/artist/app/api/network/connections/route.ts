...
export async function POST(req: Request) {
  ...
  if (existing?.status === "pending") { if (String(existing.requesterProfileId) === String(auth.context.profile._id)) await NetworkFollowModel.updateOne({ followerProfileId: auth.context.profile._id, followedProfileId: recipient._id }, { $setOnInsert: { followerProfileId: auth.context.profile._id, followedProfileId: recipient._id } }, { upsert: true }); return Response.json({ ok: true, followed: String(existing.requesterProfileId) === String(auth.context.profile._id), connection: { id: existing._id.toString(), state: String(existing.requesterProfileId) === String(auth.context.profile._id) ? "outgoing_pending" : "incoming_pending" } }); }
  ...
  try {
    ...
    await NetworkFollowModel.updateOne({ followerProfileId: auth.context.profile._id, followedProfileId: recipient._id }, { $setOnInsert: { followerProfileId: auth.context.profile._id, followedProfileId: recipient._id } }, { upsert: true });
    ...
    return Response.json({ ok: true, followed: true, connection: { id: connection!._id.toString(), state: "outgoing_pending" } }, { status: 201 });
